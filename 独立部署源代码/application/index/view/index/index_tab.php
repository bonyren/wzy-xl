...
<div id="toparea" data-options="region:'north',border:false,height:48">
	<div id="topmenu" class="d-flex flex-row justify-content-between align-items-center">
        ...
        <div class="nav-right d-flex flex-row align-items-center">
            <!------------------------------------------------------------------------------------------>
            <a href="javascript:;" onclick="baseModule.homeUrl()"
...
<!-- 内容 -->
<div id="mainarea" data-options="region:'center',split:true">
    <div id="pagetabs" class="easyui-tabs" data-options="tabPosition:'top',fit:true,border:false,plain:false,
                                                            tools:[{
                                                                iconCls:'fa fa-close',
                                                                handler:function(){
                                                                    baseModule.removeAllUrl();
                                                                }
                                                            }]">
        <div data-options="href:'<?=$urlHrefs['main']?>',title:'首页',iconCls:'fa fa-home',cache:false"></div>
    </div>
</div>
<!-- 公共部分 -->
...
window.baseModule = {
	...
	openUrl: function(node){
        ...
        var id = 'menu-tabpanel-'+node.id;
        var panel = null;
        try {
            panel = $('#'+id).panel('panel');
        } catch(e) {}
        if (panel) { //已存在
            var tabs = $('#pagetabs').tabs('tabs');
            for (var i in tabs) {
                var opts = tabs[i].panel('options');
                if (opts.id == id) {
                    $('#pagetabs').tabs('select', $('#pagetabs').tabs('getTabIndex', tabs[i]));
                    break;
                }
            }
        } else { //不存在
            $('#pagetabs').tabs('add',{
                id:id,
                title:node.attributes ? node.attributes.breadcrumb : '',
                href:node.url,
    ...
    removeAllUrl: function(){
        var tabs = $('#pagetabs').tabs('tabs');
        var len = tabs.length;
        //保留“首页"tab
        for(var i=len-1; i>0; i--){
            var tabIndex = $('#pagetabs').tabs('getTabIndex', tabs[i]);
            if(tabIndex == 0){
                //首页
                continue;
            }
            $('#pagetabs').tabs('close', tabIndex);
        }
    },
