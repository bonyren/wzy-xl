...
<div id="organizationLayout" class="easyui-layout" data-options="fit:true">
    <div data-options="region:'<?=$loginMobile?'north':'west'?>',minWidth:200" style="<?=$loginMobile?'height:40%;':'width:20%;'?>">
    ...
    <div data-options="region:'center',title:'我的组织'">
    </div>
</div>
<script>
    var organizationModule = {
        ...
        changeLevel:function(id, parentId){
            var that = organizationModule;
            var href = '<?=url('index/Organization/changeLevel')?>';
            $.messager.progress({text:'处理中，请稍候...'});
        ...
        onSelected:function(node){
            ...
            var href = '<?=url('index/Organization/customers')?>';
            href = GLOBAL.func.addUrlParam(href, 'organizationId', node.id);
